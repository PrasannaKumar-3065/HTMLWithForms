...
let render = () => {
  ...
  for (let i = 0; i < arr.length; i++) {
    a +=
      ...
      "</td> <td>" +
      arr[i].location +
      "</td> <td><button class='submit' onclick='edit(" +
      i +
      ")'>Edit</button><button class='submit' onclick='remove(" +
      i +
      ")'>Delete</button></td> </tr>";
  }
...
const dateValue = (name,id) =>{
    let date = new Date()
    let dob = new Date(name)
    if(date < dob){
        document.getElementById(id + "-error").innerHTML = "Enter a valid DOB";
    }
    else{
        document.getElementById(id + "-error").innerHTML = "";
    }
...
const validate = (object) => {
  object.style.border = "solid black 1px";
  if (object.id == "name") {
...
let check = () => {
  ...
  if (val.length == 0 && error.length == 0) {
    if (this.id == null) {
      add();
    } else {
      update();
